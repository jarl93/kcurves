...
def init_model(cfg_path, verbose=False):
    ...
    if type_dist == "points":
        if centers_init_type == "forgy": # forgy initialization
            ...
            rep_init = np.random.rand(K, input_dim)
        ...
        if centers_init_type == "knearest_kdivision":
            ...
            min = -2.0
            diff = 4.0
            s1 = np.random.rand(K, input_dim)
            s1 = min + s1 * diff
            s2 = np.random.rand(K, input_dim)
            s2 = s1 + max_length_start * s2
            rep_init = np.hstack((s1, s2))
        elif centers_init_type == "random":
            rep_init = np.random.rand(K, 2 * input_dim)
...
def PCA_proporcional_dist(X, K, percentage_K):
    ...
    for i in range(K):
        if i == 0:
            ...
            prob_X = prob_X_torch.detach().numpy()
